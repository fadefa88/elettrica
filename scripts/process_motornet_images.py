...
a.add_argument("--logo",default="assets/logopippo.jpg")
...
src_dir=Path(args.input_dir); out_dir=Path(args.output_dir)
bg_path=Path(args.background); logo_path=Path(args.logo)
if not src_dir.exists(): raise SystemExit(f"missing {src_dir}")
if not bg_path.exists() or not logo_path.exists(): raise SystemExit("missing sfondopippo.jpg or logopippo.jpg")
...
bg=im(bg_path); logo=im(logo_path); session=new_session("u2net")
done=skip=fail=0
for n,src in enumerate(files,1):
    rel=src.relative_to(src_dir).with_suffix(".png")
    cut_path=out_dir/"cutout"/rel; final_path=out_dir/"final"/rel
    cut_path.parent.mkdir(parents=True,exist_ok=True); final_path.parent.mkdir(parents=True,exist_ok=True)
    if args.force.lower()!="true" and cut_path.exists() and final_path.exists():
        skip+=1; print(f"[{n}/{len(files)}] skip {src}"); continue
    try:
        ...
        canvas=bg.copy().convert("RGBA"); cw,ch=canvas.size
        car=fit(cut,int(cw*.88),int(ch*.70))
        canvas.alpha_composite(car,((cw-car.width)//2,max(0,ch-car.height-int(ch*.07))))
        mark=fit(logo,int(cw*.18),int(ch*.16)); m=int(cw*.035)
        pos={"top-right":(cw-mark.width-m,m),"top-left":(m,m),"bottom-right":(cw-mark.width-m,ch-mark.height-m),"bottom-left":(m,ch-mark.height-m)}[args.logo_position]
        canvas.alpha_composite(mark,pos)
        canvas.save(final_path,"PNG",optimize=True)
        done+=1; print(f"[{n}/{len(files)}] ok {final_path}")
    except Exception as e:
        fail+=1; print(f"[{n}/{len(files)}] ERROR {src}: {e}",file=sys.stderr)
