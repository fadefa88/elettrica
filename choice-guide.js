(function(){
  ...
  let flowStep = 0;
  let state = { budget: 35000, km: 15000, years: 5, home: 80, priority: 'risparmio' };
  let evItems = [];
  let iceItems = [];
  let chosenEv = null;
  let chosenIce = null;
  ...
  function globalList(name){ try{ const value = eval(name); return Array.isArray(value) ? value : []; }catch(e){ return []; } }
  function callGlobal(name){ try{ const fn = eval(name); if(typeof fn === 'function') return fn(); }catch(e){} }
  function callSetStep(n){ try{ const fn = eval('setStep'); if(typeof fn === 'function') fn(n); }catch(e){} }
  ...
  function readInputs(){
    const b = byId('cgBudget'), km = byId('cgKm'), years = byId('cgYears'), home = byId('cgHome'), priority = byId('cgPriority');
    if(b) state.budget = num(b.value, state.budget);
  ...
  function askCatalog(){
    try{ if(typeof window.__motornetRequestCatalogLoad === 'function') window.__motornetRequestCatalogLoad(); }catch(e){}
    return new Promise(function(resolve){
      let ticks = 0;
      const t = setInterval(function(){
        ticks++;
        if((globalList('EV').length && globalList('IC').length) || ticks > 40){ clearInterval(t); resolve(); }
      }, 250);
    });
  }
  ...
  function ensurePage(){
    ...
    byId('cgNext').onclick = function(){ readInputs(); if(flowStep === flowSteps.length - 1) finishToReport(); else setFlowStep(flowStep + 1); };
  }
  ...
  function updateTop(){
    if(byId('cgStepLabel')) byId('cgStepLabel').textContent = (flowStep + 1) + ' di ' + flowSteps.length;
    if(byId('cgStepName')) byId('cgStepName').textContent = flowSteps[flowStep];
    if(byId('cgProgress')) byId('cgProgress').style.width = ((flowStep + 1) / flowSteps.length * 100) + '%';
    if(byId('cgPrev')) byId('cgPrev').disabled = flowStep === 0;
    if(byId('cgNext')) byId('cgNext').innerHTML = flowStep === flowSteps.length - 1 ? 'Usa queste auto e vai al report <i class="fa-solid fa-flag-checkered"></i>' : 'Avanti <i class="fa-solid fa-arrow-right"></i>';
  }
  ...
  function renderStep(){
    ...
    if(!body) return;
    if(flowStep === 0){ body.innerHTML = '<p class="eyebrow">Budget</p><h1>Quanto vuoi spendere?</h1><p class="lead">Partiamo dal limite massimo di prezzo.</p><div class="cg-grid"><label>Budget massimo €<input id="cgBudget" type="number" value="'+state.budget+'" step="1000"></label></div>'; return; }
    if(flowStep === 1){ body.innerHTML = '<p class="eyebrow">Percorrenza</p><h1>Per quanti anni e quanti km?</h1><div class="cg-grid"><label>Km annui<input id="cgKm" type="number" value="'+state.km+'" step="1000"></label><label>Anni di possesso<input id="cgYears" type="number" value="'+state.years+'" min="1" max="20"></label></div>'; return; }
    if(flowStep === 2){ body.innerHTML = '<p class="eyebrow">Ricarica</p><h1>Come caricheresti l’elettrica?</h1><div class="cg-grid"><label>Ricarica a casa %<input id="cgHome" type="number" value="'+state.home+'" min="0" max="100" step="5"></label><label>Scenario rapido<select id="cgHomePreset"><option value="80">Box o presa a casa</option><option value="50">Metà casa, metà colonnine</option><option value="20">Quasi solo colonnine</option></select></label></div>'; setTimeout(function(){ const preset = byId('cgHomePreset'); if(preset) preset.oninput = function(){ if(byId('cgHome')) byId('cgHome').value = preset.value; }; },0); return; }
    if(flowStep === 3){ body.innerHTML = '<p class="eyebrow">Priorità</p><h1>Cosa vuoi ottimizzare?</h1><div class="cg-grid"><label>Priorità<select id="cgPriority"><option value="risparmio">Risparmio totale</option><option value="prezzo">Prezzo d’acquisto basso</option><option value="autonomia">Autonomia elettrica</option><option value="prestazioni">Prestazioni</option></select></label></div>'; setTimeout(function(){ if(byId('cgPriority')) byId('cgPriority').value = state.priority; },0); return; }
    renderChoices();
  }
  ...
  async function renderChoices(){
    ...
    evItems = globalList('EV').filter(function(c){ return isElectric(c) && price(c) > 0 && price(c) <= state.budget; }).map(function(c){ return {car:c, price:price(c), cost100:evCost100(c,state,prices,charging), tco:evTco(c,state,prices,charging)}; }).sort(function(a,b){ return score(a,state.priority) - score(b,state.priority); }).slice(0,6);
    iceItems = globalList('IC').filter(function(c){ return !isElectric(c) && price(c) > 0 && price(c) <= state.budget; }).map(function(c){ return {car:c, price:price(c), cost100:iceCost100(c,prices), tco:iceTco(c,state,prices)}; }).sort(function(a,b){ return score(a,state.priority) - score(b,state.priority); }).slice(0,6);
    chosenEv = chosenEv || (evItems[0] && evItems[0].car) || null;
    chosenIce = chosenIce || (iceItems[0] && iceItems[0].car) || null;
    if(!evItems.length && !iceItems.length){ body.innerHTML = '<p class="eyebrow">Proposte</p><h1>Nessuna auto trovata entro questo budget.</h1><p class="lead">Aumenta il budget o torna al comparatore classico.</p>'; return; }
    let verdict = '';
    if(evItems[0] && iceItems[0]){ const diff = iceItems[0].tco - evItems[0].tco; verdict = '<div class="cg-verdict"><b>'+(diff >= 0 ? 'La migliore elettrica costa meno nel periodo indicato.' : 'La migliore termica costa meno nel periodo indicato.')+'</b><span>Differenza stimata: '+euro0.format(Math.abs(diff))+' in '+state.years+' anni.</span></div>'; }
    body.innerHTML = '<p class="eyebrow">Scelta</p><h1>Scegli una coppia da confrontare.</h1><p class="lead">Dopo il finish arrivi allo stesso report del comparatore classico.</p>'+verdict+'<div class="cg-cols"><section><h3>Elettriche</h3>'+evItems.map(function(i){return renderCard(i,'ev');}).join('')+'</section><section><h3>Termiche</h3>'+iceItems.map(function(i){return renderCard(i,'ice');}).join('')+'</section></div>';
    body.querySelectorAll('.cg-card').forEach(function(btn){ btn.onclick = function(){ const kind = btn.dataset.kind; const id = btn.dataset.id; const src = kind === 'ev' ? evItems : iceItems; const found = src.find(function(i){ return i.car.id === id; }); if(found){ if(kind === 'ev') chosenEv = found.car; else chosenIce = found.car; renderChoices(); } }; });
  }

  function setCheck(id, val){ const e = byId(id); if(e) e.checked = !!val; }
  function setVal(id, val){ const e = byId(id); if(e) e.value = val; }
  function setHiddenSelect(id, val){ const e = byId(id); if(!e) return; e.innerHTML = '<option value="'+esc(val)+'" selected>'+esc(val)+'</option>'; e.value = val; }
  function finishToReport(){
    ...
    setHiddenSelect('evSelect', chosenEv.id); setHiddenSelect('iceSelect', chosenIce.id);
    setVal('evModelSearch', carName(chosenEv)); setVal('iceModelSearch', carName(chosenIce));
