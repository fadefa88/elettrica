...
def full_url(u): return urllib.parse.urljoin(BASE, u.split("#")[0])
def make_id(url, code): return "autoit_" + code.lower() + "_" + hashlib.sha1((code + "|" + url).encode()).hexdigest()[:12]
...
def specs(text):
    text = clean(text); out = {}
    patterns = [("power_kw", r"(\d+(?:,\d+)?)\s*kW", float), ("power_cv", r"(\d+(?:,\d+)?)\s*CV", int), ("consumption_kwh_100km", r"(\d+(?:,\d+)?)\s*kWh\s*/\s*100\s*km", float), ("consumption_l_100km", r"(\d+(?:,\d+)?)\s*l\s*/\s*100\s*km", float), ("consumption_kg_100km", r"(\d+(?:,\d+)?)\s*kg\s*/\s*100\s*km", float), ("battery_kwh", r"(\d+(?:,\d+)?)\s*kWh(?!\s*/)", float), ("range_wltp_km", r"(?:autonomia|WLTP|range)[^\d]{0,25}(\d{2,4})\s*km", int), ("emissions_g_km", r"(\d{1,3})\s*g\s*/\s*km", int)]
    for key, rx, cast in patterns:
        m = re.search(rx, text, re.I)
        if m:
            val = float(m.group(1).replace(",", ".")); out[key] = int(val) if cast is int else round(val, 1)
    if "power_cv" in out and "power_kw" not in out: out["power_kw"] = round(out["power_cv"] * 0.7355, 1)
    return out
def walk(o):
...
def discover_links(html):
    ...
    for a in soup.find_all("a", href=True):
        url = full_url(a["href"]); path = urllib.parse.urlparse(url).path
        if "/listino-nuovo/" in path and "/search" not in path and url not in links: links.append(url)
    return links
def parse_detail(html, url, code, label, category):
    ...
    for block in json_blocks(soup):
        for d in walk(block):
            ...
            if not any(w in blob.lower() for w in ["brand", "model", "price", "prezzo", "kw", "cv"]): continue
            b = d.get("brand") or d.get("manufacturer") or d.get("make")
            if isinstance(b, dict): b = b.get("name")
            if b and not car["brand"]: car["brand"] = clean(b)
            m = d.get("model") or d.get("name")
            if m and car["model"] == title: car["model"] = clean(m)
            ...
            if p and not car.get("price_eur"):
                try: car["price_eur"] = int(float(str(p).replace(",", ".")))
                except Exception: pass
            for k, v in specs(blob).items(): car.setdefault(k, v)
    for k, v in specs(text).items(): car.setdefault(k, v)
    if not car.get("price_eur"):
        p = parse_price(text)
        if p: car["price_eur"] = p
    if not car["brand"]:
        parts = title.split(); car["brand"] = parts[0] if parts else ""
    return car if car["brand"] and car["model"] else None
def main():
    ap = argparse.ArgumentParser(); ap.add_argument("--limit", type=int, default=80); ap.add_argument("--pages-per-fuel", type=int, default=2); ap.add_argument("--delay", type=float, default=4); ap.add_argument("--timeout", type=int, default=30); args = ap.parse_args()
    rp = urllib.robotparser.RobotFileParser(); rp.set_url(BASE + "/robots.txt")
    try: rp.read()
    except Exception as e: print("WARN robots non leggibile:", e)
    session = requests.Session(); session.headers.update({"User-Agent": UA, "Accept-Language": "it-IT,it;q=0.9,en;q=0.6"})
    cars, errors, seen = [], [], set()
    for code, (label, category, list_url) in SOURCES.items():
        if len(cars) >= args.limit: break
        if not rp.can_fetch(UA, list_url): errors.append({"url": list_url, "error": "blocked_by_robots"}); continue
        try:
            time.sleep(args.delay); r = session.get(list_url, timeout=args.timeout)
            if r.status_code == 429: raise RuntimeError("429 Too Many Requests")
            r.raise_for_status()
        except Exception as e: errors.append({"url": list_url, "error": str(e)}); continue
        for link in discover_links(r.text):
            if len(cars) >= args.limit: break
            if link in seen: continue
            seen.add(link)
            if not rp.can_fetch(UA, link): errors.append({"url": link, "error": "blocked_by_robots"}); continue
            try:
                time.sleep(args.delay); d = session.get(link, timeout=args.timeout)
                if d.status_code == 429: raise RuntimeError("429 Too Many Requests")
                d.raise_for_status(); car = parse_detail(d.text, link, code, label, category)
                if car: cars.append(car)
            except Exception as e:
                errors.append({"url": link, "error": str(e)})
                if "429" in str(e): break
    OUT.parent.mkdir(parents=True, exist_ok=True)
    payload = {"source": "auto.it", "status": "ok" if cars else "empty", "scraped_at": now(), "schema": "cars_autoit_v1", "cars": cars, "errors": errors[-100:]}
